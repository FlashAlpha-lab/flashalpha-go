...
// Typed response model for `GET /v1/volatility/{symbol}` (Growth+).
//
...
// strikes/expiries, sparse OI, market closed). Slice fields default to
...
type VolatilityResponse struct {
	...
	// RealizedVol is the trailing-window realized-vol ladder. See VolatilityRealized.
	RealizedVol *VolatilityRealized `json:"realized_vol"`
	// AtmIv is the at-the-money implied volatility (annualised %, e.g. 18.5 = 18.5%).
...
type VolatilityIvRvSpreads struct {
	...
	// (e.g. "iv_rich", "iv_cheap", "fair"). Safe to surface verbatim.
...
// (downside-skewed smile, the typical equity-index pattern). SmileRatio and
// TailConvexity describe the higher-order shape of the smile.
